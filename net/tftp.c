...
void tftp_start(enum proto_t protocol)
{
	...
	if (net_boot_file_name[0] == '\0') {
		...
		char *p = strchr(net_boot_file_name, ':');//p-->":"

		if (p == NULL) {
			strncpy(tftp_filename, net_boot_file_name, MAX_LEN);
			tftp_filename[MAX_LEN - 1] = 0;//"0" is equal to empty in ASCII
		} else {
			tftp_remote_ip = string_to_ip(net_boot_file_name);//net_boot_file_name 既包含IP又包含文件名
...
#endif
	{
		...
		puts("Loading: *\b");
		tftp_state = STATE_SEND_RRQ;//TFTP第一阶段，请求下载数据
	...
	net_set_timeout_handler(timeout_ms, tftp_timeout_handler);//初始化超时控制函数
	net_set_udp_handler(tftp_handler);//初始化数据接受发送，写入内存的函数
